...
int main(int argc, char** argv)
{
    ...
    auto test_labeling = [&](node* p, const std::array<bool, 3> l, const bool result)
    {
    ...
    node* and_not01 = mgr.or_rec(mgr.negate(mgr.projection(0)), mgr.negate(mgr.projection(1)));
    ...
    node* and_not02 = mgr.or_rec(mgr.negate(mgr.projection(0)), mgr.negate(mgr.projection(2)));
    ...
    /*
    node* and_not12 = mgr.or_rec(mgr.negate(mgr.projection(1)), mgr.negate(mgr.projection(2)));
    ...
    node* simplex = mgr.and_rec(mgr.and_rec(mgr.and_rec(or_012, and_not01), and_not02), and_not12);
